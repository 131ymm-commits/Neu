...
(async () => {
  ...
  for (const iso of isos) {
    ...
    const r = await A.evaluate(async k => { const d = await createPatch({ tax: 1 }, 'W', 3, ''); const pid = d && d.pid; const seen = !!M.c.patches[pid]; await adopt(k, pid, d); return { pid, seen }; }, 'C:' + iso);
  ...
  const pidK = await Bp.evaluate(() => createPatch({ invest: 1 }, 'W', 2, 'в гонке').then(d => d && d.pid));
  ...
  const chkK = await A.evaluate(p => { const out = []; for (const y in M.Y) { (((M.Y[y].C || {}).chk) || []).concat(((M.Y[y].C || {}).done) || []).forEach(k => { if (k.pid === p) out.push(y + ':' + (k.ok === undefined ? 'идёт' : k.ok)); }); } return out; }, pidK);
  ...
  const bad = await Bp.evaluate(async () => { const p = await createPatch({ social: -1 }, 'W', 1, 'плохой'); return p && p.pid; });
  await Bp.waitForTimeout(100); await Bp.evaluate(p => adopt('C:UZB', p), bad); await Bp.waitForTimeout(100);
  const good = await Bp.evaluate(() => createPatch({ social: 1 }, 'u', 3, 'новый').then(d => d && d.pid));
  await Bp.waitForTimeout(100);
  ...
  const doneBad = await A.evaluate(p => { for (const y in M.Y) for (const d of (((M.Y[y].C || {}).done) || [])) if (d.pid === p) return d.ok; return null; }, bad);
