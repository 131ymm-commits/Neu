...
def main(arm, seed):
    ...
    for i in range(STEPS):
        ...
        if arm != 'A0':
            ...
            aux = (((head(out.hidden_states[-1][:, :P]) - y) ** 2).reshape(-1, K).mean(0) / var).mean()
            loss = loss + LAM * aux
    ...
    os.makedirs(os.path.join(HERE, 'auto'), exist_ok=True)
    json.dump(log, open(os.path.join(HERE, f'auto/{arm}_s{seed}.json'), 'w'))
