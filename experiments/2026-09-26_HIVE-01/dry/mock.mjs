...
async function agent(prompt, o) {
  ...
  if (P.ballot) return { ballot: [{ kind: 'добавить закон', law_number: 0, text: 'закон ' + n, summary: 's' }, { kind: 'протокол', law_number: 0, text: 'пересмотр вкл', summary: 's' }, { kind: 'удалить закон', law_number: 1, text: '', summary: 's' }] }
...
for (const [k, v] of Object.entries(out.history)) if (v.legislation) console.log(k, 'законов', v.hive.laws.length, 'принято', v.legislation.adopted.length, 'дельфи', v.hive.delphi, v.hive.agg)
