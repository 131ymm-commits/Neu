# AUTO-05: копия auto_train.py AUTO-01..03 + ветвь AR (цель мировой головы взята у соседней последовательности батча).
...
def total_loss(params, seq, use_self, lam, shuffle):
    p, head = params
    ...
    y = jnp.where(shuffle > 0, jnp.roll(y, 1, axis=0), y)   # AR: цель не связана с контекстом
...
@jax.jit
def step(params, st, seq, use_self, lam, shuffle):
    (l, (ce, aux)), g = jax.value_and_grad(total_loss, has_aux=True)(params, seq, use_self, lam, shuffle)
...
mode = {'A0': 0.0, 'AW': 0.0, 'AR': 0.0, 'AS': 1.0, 'AA': 0.0}[arm]
log = []
t0 = time.time()
for i in range(STEPS + 1):
    ...
    params, st, ce, aux = step(params, st, jnp.asarray(seq), jnp.float32(mode), jnp.float32(lam), jnp.float32(arm == 'AR'))
    if i % 100 == 0:
