export const meta = {
  name: 'hive2-step',
  description: 'HIVE-02: шаг эволюции ульев со смертью — «ответ» (головы отвечают на вопросы без правды) или «рождение» (голова-мутатор пишет законы потомка по законам родителей и их итогам; предметные законы отсеиваются кодом)',
  phases: [{ title: 'Шаг' }],
}
...
const CHILD = obj({ laws: { type: 'array', items: { type: 'string' }, description: 'законы потомка, не больше 8' }, rationale: { type: 'string' } }, ['laws', 'rationale'])
async function birth(name, c) {
  const P = (h, t) => `${t}:\n` + h.laws.map((x, i) => `${i + 1}. ${x}`).join('\n')
  const r = await call(`${name}:birth`, `${A.no_tools}\n\nТы — мутатор в эволюции ульев. Улей — три головы, которые оценивают трудные величины (оценка и 80 % интервал) и работают по общим законам. ` +
    'Худшие ульи умирают, лучшие дают потомство. Напиши законы потомка.\n\n' + P(c.parent, 'Законы родителя') + (c.partner ? '\n\n' + P(c.partner, 'Законы второго родителя (скрещивание: возьми лучшее от обоих)') : '') +
    `\n\nИтоги родителя в последнем поколении (правда и ответы):\n${c.feedback}\n\n` +
    `Сохрани то, что работает, и внеси ${c.partner ? 'скрещивание и' : ''} одну-две мутации: измени, добавь или удали закон так, чтобы потомок был точнее на НОВЫХ вопросах того же рода. ` +
    'Законы — только о том, как головы работают и взаимодействуют: без чисел, формул, названий типов задач и приёмов счёта для конкретного типа задач (такие законы отбрасываются автоматически). Не больше 8 законов.',
    CHILD, r => Array.isArray(r.laws) && r.laws.length >= 1 ? null : 'формат')
  const laws = (r?.laws || []).filter(t => !subjective(t)).slice(0, 8)
  return { laws: laws.length ? laws : c.parent.laws, rejected: (r?.laws || []).filter(t => subjective(t)), rationale: r?.rationale }
}

let out = {}
if (A.mode === 'birth') {
  const names = Object.keys(A.children)
  const res = await parallel(names.map(nm => () => birth(nm, A.children[nm])))
  names.forEach((nm, i) => out[nm] = res[i])
} else if (A.mode === 'answer') {
