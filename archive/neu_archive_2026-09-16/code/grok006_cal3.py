...
on = []
for e in ends:
    if not on or e - on[-1] >= 10: on.append(int(e) + 1000)   # absolute step index (post starts at t=1000)
targets = {}
for o in on[3:9]:
...
for t in range(max(targets) + 1):
    ...
    if t in targets:
        lam, vprev = sharpness(W1, W2, vprev, iters=30)
        g1, g2, L = grad(W1, W2)
        out.append((t, targets[t], lam, L))
        print(f"t={t} {targets[t]}: lambda_max={lam:.4f} lambda*lr={lam*lr:.3f} loss={L:.4f} [{time.time()-t0:.0f}s]", flush=True)
    g1, g2, _ = grad(W1, W2)
    W1 -= lr * (g1 + wd * W1); W2 -= lr * (g2 + wd * W2)
np.save("/home/claude/grok006_cal3_413.npy", np.array([(a, {"pre":0,"burst":1,"mid":2,"before_switch":3}[b], c, dd) for a, b, c, dd in out]))
