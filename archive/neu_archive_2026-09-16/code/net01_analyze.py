"""NET-01 — анализ: три разбиения одной траектории (A: сеть IS, B: 100 микросостояний по расстояниям, C: 10 бинов E_IS); ранг моды
«твёрдое/жидкое» (max |corr| собственного вектора с индикатором икосаэдра), ζ, решение по точной форме (p_rank1); контроли: перестановка
блоками, сходимость по половине кадров. По PREREG 2026-09-08."""
...
TAU = 2; KTS = 14; NS = 200; OUT = '/home/claude/net01_results.json'; res = json.load(open(OUT)) if os.path.exists(OUT) else {}; t0 = time.time()
...
for fp in sorted(glob.glob('/home/claude/net01_T*_s*.npz')):
    ...
    d = np.load(fp); Eis = d['Eis']; D = d['D'].astype(float); ind = (Eis <= -44.32).astype(float)
    labA = np.unique(np.round(Eis, 3), return_inverse=True)[1]
    labB = microstates(D, 100, seed=0)
    edges = np.quantile(Eis, np.linspace(0, 1, 11)[1:-1]); labC = np.searchsorted(edges, Eis)
    print(f"{name}: кадров {len(Eis)}, IS {len(np.unique(labA))}, доля икосаэдра {ind.mean():.3f}", flush=True)
    r = dict(n=int(len(Eis)), n_is=int(len(np.unique(labA))), frac_ico=float(ind.mean()))
    r['A'] = analyze(labA, ind, 'A: сеть IS'); r['B'] = analyze(labB, ind, 'B: наблюдаемая'); r['C'] = analyze(labC, ind, 'C: бины E_IS')
    # контроль (c): блочная перестановка кадров (блоки по 10)
    rng = np.random.default_rng(0); nb = len(Eis) // 10; perm = rng.permutation(nb); idx = (perm[:, None] * 10 + np.arange(10)).ravel()
    r['A_shuf'] = analyze(labA[idx], ind[idx], 'A перестановка'); r['B_shuf'] = analyze(labB[idx], ind[idx], 'B перестановка')
    # контроль (d): сходимость — первая половина кадров
    h = len(Eis) // 2; r['A_half'] = analyze(labA[:h], ind[:h], 'A половина'); r['B_half'] = analyze(labB[:h], ind[:h], 'B половина')
