"""EXT-001c — uniform windows with calibration-object validity rule. Per PREREG (frozen 2026-08-26)."""
import json, os, sys, time
import numpy as np

exec(open("/home/claude/ext001_run.py").read().split("t0 = time.time()")[0])

CKPT_C = "/home/claude/ext001c_ckpt.json"
t0 = time.time()
ck = json.load(open(CKPT_C)) if os.path.exists(CKPT_C) else {}
...
def valid_object(w, i_on):
    """Frozen validity: L80 crossed inside window AND post-median > E-median + 2*sE."""
    from scipy.ndimage import median_filter
    N = len(w)
    E = w[:max(i_on - 5, 10)]
    post = w[i_on + 5: min(i_on + 20, N)]
    if len(post) < 5: post = w[i_on:]
    t = np.arange(len(E))
    res = E - np.polyval(np.polyfit(t, E, 1), t)
    sE = 1.4826 * np.median(np.abs(res - np.median(res))) + 1e-12
    mE, mP = np.median(E), np.median(post)
    if mP - mE < 2 * sE: return False
    xm = median_filter(w.astype(float), size=3, mode="nearest")
    L80 = mE + 0.8 * (mP - mE)
    return bool((xm >= L80).any())

def draw_valid(name, seed0, redraws):
    for k in range(10):
        w, ion = cme_window40(name, True, seed0 + 1000 * k)
        if w is not None and valid_object(w, ion):
            if k: redraws.append((name, seed0, k))
            return w, ion
    return None, None
...
if "th" not in ck:
    ...
    for name, bag in (("fold", Sf), ("trans", St)):
        for i in (1, 2, 3):
            w, ion = draw_valid(name, 994000 + i + (0 if name == "fold" else 100), redraws)
    ...
    for name in ("fold", "trans"):
        for i in (1, 2, 3):
            w, ion = cme_window40(name, False, 994500 + i + (0 if name == "fold" else 100))
            f = feats(w, ion)
            rises.append(f["rise"]); zs.append(f["z"])
    ok = all(s > 0 for s in Sf + St) and len(Sf) == 3 and len(St) == 3
    ...
    ck["th"] = dict(thS=thS, S_alive=bool(alive), minS_fold=(min(Sf) if Sf else None),
                    ...
                    redraws=redraws)
    json.dump(ck, open(CKPT_C, "w"), default=float)
    print(f"ЗАМОРОЖЕНО: S_alive={alive} thS={thS} ({(min(Sf) if Sf else None)}/{(max(St) if St else None)}) "
          f"thRise={thRise:.2f} thR2={thR2:.2f} | замен {len(redraws)}", flush=True)
th = ck["th"]
if not th["S_alive"]:
    print("S-ножка мертва и с валидными объектами — NGRIP не вскрывается (записано)."); sys.exit(0)
...
if "valid" not in ck:
    ...
    for name, want in (("fold", "фолд"), ("trans", "непрерывный")):
        for i in (1, 2, 3, 4, 5):
            w, ion = draw_valid(name, 995000 + i + (0 if name == "fold" else 100), redraws)
            if w is None: continue
            lab = classify(feats(w, ion), th)
            correct += (lab == want)
            print(f"валид {name}#{i}: -> {lab}", flush=True)
    for j, name in enumerate(("fold", "trans", "fold", "trans", "fold", "trans")):
        w, ion = cme_window40(name, False, 995500 + j)
        lab = classify(feats(w, ion), th)
        ctl_ok += (lab == "нет рождения")
    pf1 = correct >= 8 and ctl_ok >= 5
    ck["valid"] = dict(correct=correct, ctl_ok=ctl_ok, redraws=redraws, PF1=bool(pf1),
                       PF1_killed=bool(correct <= 6 or ctl_ok <= 3))
    json.dump(ck, open(CKPT_C, "w"), default=float)
    print(f"P-F1: {correct}/10, контроли {ctl_ok}/6, замен {len(redraws)} -> "
          f"{'ПОДТВЕРЖДЁН' if pf1 else 'не пройден — стоп'}", flush=True)
if not ck["valid"]["PF1"]:
    sys.exit(0)
...
if "ngrip" not in ck:
    ...
    json.dump(ck, open(CKPT_C, "w"), default=float)
    print(f"\nМетки: {lab_counts}")
    print(f"P-F2 (фолд среди 11 ранее-полных): {keep_full}/11 -> "
          f"{'ПОДТВЕРЖДЁН' if pf2 else ('УБИТ' if pf2_kill else 'не установлен')} (LOO={loo}; из прежних 8 сохранили {keep})")
