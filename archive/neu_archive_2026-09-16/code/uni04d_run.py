"""UNI-04d — oscillator label = winding AND O2' amplitude persistence. Per PREREG (frozen 2026-08-27)."""
import json, time
import numpy as np
...
def wind_raw(seg, lag=1, nsur=200):
    ...
    if res.std() == 0: return None, None, False, 0.0
    u = res[lag:]; v = res[:-lag]
    phi = np.arctan2(v, u)
    ...
    return z, th, bool(z >= th), float(np.sqrt(np.mean(res ** 2)))

def label3(traj):
    seg = traj[-4000:]
    z, th, fire, rms = wind_raw(seg)
    xbar = max(float(np.mean(seg)), 1e-6)
    poiss = np.sqrt(xbar / V)
    amp_ok = rms >= 10 * poiss
    if fire and amp_ok: return "осциллятор", z, rms / poiss
    ...
    if cr.size and cr[0] >= 20:
        ...
        return classify(feats(w, i_on - lo), TH), z, rms / poiss
    w = blocks[-100:]
    return classify(feats(w, 80), TH), z, rms / poiss
...
for tau, want in ((0.05, "фолд"), (0.40, "осциллятор")):
    ...
    cn = sum(1 for r in range(3) if label3(ct[r])[0] == "нет рождения")
...
json.dump(dict(res=res, PUM1d=bool(pum1), PUM1d_killed=bool(pum1_kill)),
          open("/home/claude/uni04d_results.json", "w"), default=float)
print(f"\nP-UM1d: фолд-арм {ok_lo}/5, осциллятор-арм {ok_hi}/5 -> "
